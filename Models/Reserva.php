...
class Reserva extends Core\Model
{
	...
	public function getSuitesDisponiveis($dataInicio, $dataFim)
	{
		try
		{
	...
	public function excluirReserva($ReservaE)
	{
		try
		{
			$sql = $this->db->prepare("DELETE FROM reservas WHERE rsv_id = :rsv_id;",
				array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
	...
	public function consultarReservas()
	{
		try
		{
			$sql = $this->db->prepare("SELECT * FROM reservas;");
			$sql->execute();
			return $sql->fetchAll(PDO::FETCH_ASSOC);
		}
		catch(Exception $exception)
	...
	public function consultarReservasSearch($searchText, $orderColumn, $orderDir, $start, $rows)
	{
		...
		$strSQL = "SELECT
			...
			ste_tipo,
			(
				(DATEDIFF(rsv_data_saida, rsv_data_entrada) * ste_valor)
				+
